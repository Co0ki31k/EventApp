<?php
/**
 * API endpoint - dodawanie podkategorii (tylko admin)
 */
...
if (!isset($data['category_id']) || !isset($data['name']) || trim($data['name']) === '') {
    ...
    echo json_encode(['success' => false, 'error' => 'ID kategorii i nazwa podkategorii są wymagane']);
    exit;
}
...
try {
    ...
    if ($inserted) {
        echo json_encode([
            'success' => true,
            'message' => 'Podkategoria została dodana'
        ]);
    } else {
        throw new Exception('Nie udało się dodać podkategorii');
    }
